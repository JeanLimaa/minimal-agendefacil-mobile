...
import {
  ...
  FormDataType,
  GenericFormField,
} from "@/modules/settings/components/GenericForm";
...
export default function WeeklyScheduleField({
  ...
  const [errors, setErrors] = useState<{
    [day: string]: ValidationError | null;
  }>({});
  ...
  const getScheduleByDay = (
    dayOfWeek: number
  ): DailyWorkingHour | undefined => {
    return currentSchedule.find(
      (entry: DailyWorkingHour) => entry.dayOfWeek === dayOfWeek
    );
  };
  ...
  const validateDay = (
    ...
    if (startTime && !endTime) {
      return {
        type: "missing_end",
        message: "Informe também o horário de fim",
      };
    }
    ...
    if (endTime && !startTime) {
      return {
        type: "missing_start",
        message: "Informe também o horário de início",
      };
    }
    ...
    if (startTime && endTime) {
      if (!isValidTimeRange(startTime, endTime)) {
        return {
          type: "invalid_range",
          message: "Horário de início deve ser menor que o de fim",
        };
      }
    }
  ...
  const getActiveDaysCount = () => {
    return currentSchedule.filter((entry) => entry.startTime && entry.endTime)
      .length;
  };
      ...
      return (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyStateText}>
            {field.placeholder ||
              "Nenhum horário definido. Será utilizado o horário da empresa."}
          </Text>
        </View>
      );
    }
      ...
      <View style={styles.summaryContainer}>
        <View style={styles.summaryHeader}>
          <Text style={styles.summaryTitle}>
            {activeDays}{" "}
            {activeDays === 1 ? "dia configurado" : "dias configurados"}
          </Text>
        </View>
            ...
            return (
              isActive && (
                <View
                  ...
                  style={[
                    styles.summaryDay,
                    isActive && styles.summaryDayActive,
                  ]}
                >
                  <Text
                    style={[
                      styles.summaryDayLabel,
                      isActive && styles.summaryDayLabelActive,
                    ]}
                  >
                    {day.label}
                  </Text>
        ...
        <View style={{ gap: 16, marginBottom: 96 }}>
          ...
          <View style={{ gap: 12 }}>
            {weekdays.map((day) => {
              ...
              const hasError =
                errors[day.key] !== null && errors[day.key] !== undefined;
